...
let _checkNpmVersionAsync = (() => {
  var _ref = _asyncToGenerator(function* (projectRoot) {
    try {
      ...
      if ((_semver || _load_semver()).default.lt(npmVersion, MIN_NPM_VERSION) || (_semver || _load_semver()).default.gte(npmVersion, BAD_NPM_VERSION)) {
        (_ProjectUtils || _load_ProjectUtils()).logError(projectRoot, 'expo', `Error: You are using npm version ${npmVersion}. Please use an npm version that is >= ${MIN_NPM_VERSION} and < ${BAD_NPM_VERSION}.`, 'doctor-npm-version');
        return FATAL;
      } else {
        (_ProjectUtils || _load_ProjectUtils()).clearNotification(projectRoot, 'doctor-npm-version');
      }
    } catch (e) {
      (_ProjectUtils || _load_ProjectUtils()).logWarning(projectRoot, 'expo', `Warning: Could not determine npm version. Make sure your version is >= ${MIN_NPM_VERSION} and < ${BAD_NPM_VERSION}.`, 'doctor-npm-version');
      return WARNING;
    }
