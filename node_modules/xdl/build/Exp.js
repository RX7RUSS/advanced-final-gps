...
let _downloadStarterAppAsync = (() => {
  var _ref2 = _asyncToGenerator(function* (templateId, progressFunction, retryFunction) {
    ...
    yield (_Api || _load_Api()).default.downloadAsync(url, _path.default.join(_starterAppCacheDirectory(), filename), {
      progressFunction,
      retryFunction
    });
    return {
...
let extractTemplateApp = exports.extractTemplateApp = (() => {
  var _ref4 = _asyncToGenerator(function* (starterAppPath, name, root) {
    ...
    // Update app.json
    let appJson = yield _fs.default.readFile.promise(_path.default.join(root, 'app.json'), 'utf8');
    let customAppJson = appJson.replace(/\"My New Project\"/, `"${data.name}"`).replace(/\"my-new-project\"/, `"${data.name}"`);
    yield _fs.default.writeFile.promise(_path.default.join(root, 'app.json'), customAppJson, 'utf8');
